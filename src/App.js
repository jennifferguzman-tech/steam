...
// CONSTANTES
// ─────────────────────────────────────────────────────────────────────────────
const catColor  = {"Ciencia":"#FF6B6B","Ingeniería":"#4ECDC4","Arte + Ciencia":"#A78BFA","Tecnología":"#F59E0B","Matemáticas":"#10B981","Arte + Tecnología":"#EC4899"};
...
function Spinner(){return(<div style={{display:"flex",alignItems:"center",justifyContent:"center",padding:"80px 0",flexDirection:"column",gap:16}}><div style={{width:40,height:40,border:"4px solid #f0f0f0",borderTop:"4px solid #FF6B6B",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/><div style={{color:"#bbb",fontWeight:700,fontSize:14}}>Cargando actividades...</div></div>);}
...
function PublicApp() {
  ...
  const getStepImg = (s,a) => s.image ? <img src={s.image} alt={s.title} style={{width:"100%",height:"100%",objectFit:"cover"}}/> : defaultStepSvg(s.number,a.category_color||"#FF6B6B");
  ...
  return(
    <div style={{minHeight:"100vh",background:"#FDFAF6",fontFamily:"'Nunito',sans-serif"}}>
      {/* Header */}
      <div style={{background:"linear-gradient(135deg,#FF6B6B 0%,#FF8E53 50%,#FFC75F 100%)",padding:"44px 24px 60px",textAlign:"center",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative",zIndex:1}}>
          ...
          <h1 style={{fontFamily:"'Fredoka One',cursive",fontSize:"clamp(26px,6vw,48px)",color:"white",textShadow:"0 2px 20px rgba(0,0,0,.15)",marginBottom:6}}>{selected?selected.title:"Actividades STEAM"}</h1>
          <p style={{color:"rgba(255,255,255,.9)",fontSize:16,fontWeight:700}}>{selected?`${selected.emoji} ${selected.category} · ${selected.time} · ${selected.age}`:"Ciencia · Tecnología · Ingeniería · Arte · Matemáticas"}</p>
        </div>
      </div>
      ...
      <div style={{maxWidth:1200,margin:"0 auto",padding:"0 20px 60px",marginTop:-20}}>
        ...
        {!loading&&!selected&&(
          <div style={{display:"flex",gap:22,alignItems:"flex-start"}}>
              ...
              <div style={{background:"white",borderRadius:20,boxShadow:"0 4px 20px rgba(0,0,0,.08)",overflow:"hidden"}}>
                ...
                {!sideOpen&&(
                  <div style={{padding:"12px 0",display:"flex",flexDirection:"column",alignItems:"center",gap:10}}>
                    {[{icon:"🔬",v:cat,d:"Todas"},{icon:"👶",v:age,d:"Todas las edades"},{icon:"⭐",v:diff,d:"Todas"}].map((f,i)=>(
                      <div key={i} title={f.v} style={{width:32,height:32,borderRadius:8,background:f.v!==f.d?"#FF6B6B18":"#F9FAFB",border:f.v!==f.d?"2px solid #FF6B6B":"2px solid #E5E7EB",display:"flex",alignItems:"center",justifyContent:"center",fontSize:15}}>{f.icon}</div>
                    ))}
                ...
                {sideOpen&&(
                  <div style={{padding:"16px 16px 4px"}}>
                    <FilterSection sectionKey="cat"  label="🔬 Categoría" items={CATEGORIES}   val={cat}  set={setCat}  color="#FF6B6B"/>
                    ...
                    {activeFilters.length>0&&(
                      <div style={{paddingBottom:14}}>
                        ...
                        <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:10}}>
                          {activeFilters.map((f,i)=><button key={i} onClick={f.clear} style={{display:"flex",alignItems:"center",gap:5,background:"#FF6B6B18",color:"#FF6B6B",border:"1.5px solid #FF6B6B55",borderRadius:50,padding:"4px 10px",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"'Nunito',sans-serif"}}>{f.label} ×</button>)}
                        </div>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              {filtered.length===0&&<div style={{textAlign:"center",padding:"60px 0",background:"white",borderRadius:20,boxShadow:"0 4px 20px rgba(0,0,0,.06)"}}><div style={{fontSize:52,marginBottom:12}}>🔍</div><button onClick={resetAll} style={{padding:"10px 26px",borderRadius:50,border:"none",background:"#FF6B6B",color:"white",fontFamily:"'Nunito',sans-serif",fontWeight:800,fontSize:14,cursor:"pointer"}}>Limpiar filtros</button></div>}
        ...
        {!loading&&selected&&(
          <div>
            <div style={{background:"white",borderRadius:18,padding:22,marginBottom:24,boxShadow:"0 4px 20px rgba(0,0,0,.08)",borderLeft:`6px solid ${selected.category_color||"#FF6B6B"}`}}>
            ...
            <h2 style={{fontFamily:"'Fredoka One',cursive",fontSize:24,color:"#2D2D2D",marginBottom:20}}><span style={{color:selected.category_color||"#FF6B6B"}}>📋</span> Paso a paso</h2>
            {(selected.steps||[]).map(step=>(
              <div key={step.number} style={{background:"white",borderRadius:18,overflow:"hidden",boxShadow:"0 4px 20px rgba(0,0,0,.08)",marginBottom:18,border:"2px solid #f0f0f0",display:"flex"}}>
                <div style={{width:52,minWidth:52,background:selected.category_color||"#FF6B6B",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Fredoka One',cursive",fontSize:26,color:"white"}}>{step.number}</div>
            ...
            <div style={{textAlign:"center",marginTop:30}}>
              <button onClick={close} style={{background:selected.category_color||"#FF6B6B",color:"white",border:"none",borderRadius:50,padding:"13px 30px",fontSize:15,fontWeight:800,cursor:"pointer",fontFamily:"'Nunito',sans-serif"}}>← Ver todas las actividades</button>
            </div>
          </div>
        )}
      </div>
